  import type { AuthedSocket, SendMessageEvent, RawClientEvent , SubscribeRoomsEvent, UnsubscribeRoomEvent, ClientEvent, } from "./types.js";
      ...
      try {
        ...
        switch (event.type) {
          case "SUBSCRIBE_ROOMS":
            handleSubscribeRooms(socket, event as SubscribeRoomsEvent);
            break;

          case "UNSUBSCRIBE_ROOM":
            handleUnsubscribeRoom(socket, event as UnsubscribeRoomEvent);
            break;
  ...
  function handleSubscribeRooms(socket: AuthedSocket, event: SubscribeRoomsEvent) {
    const { roomIds } = event;

    if (!Array.isArray(roomIds)) return;  // Validate that roomIds is an array before proceeding

    roomManager.subscribe(socket, roomIds);
  ...
  function handleUnsubscribeRoom(socket: AuthedSocket, event: UnsubscribeRoomEvent) {
    const { roomId } = event;
  ...
  async function handleSendMessage(socket: AuthedSocket, event: SendMessageEvent) {
